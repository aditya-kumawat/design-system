...
export type fetchDataFn = (attr: FetchDataAttr) => Promise<{
  totalRecords: number,
  data: Data[],
  schema: CellSchema[]
}>;
export type updateSelectAllFn = (attr: TableState['selectAll']) => void;
export type updateCellSchemaFn = (name: string, schemaUpdate: Partial<StateSchema>) => void;
export type updateRowDataFn = (rowIndexes: number[], dataUpdate: Partial<StateData>) => void;
export type updateReorderHighlighterFn = (dim: number | null) => void;
...
export interface TableProps {
   ...
   */
  statusMapper?: Record<string, StatusHintsProps['appearance']>;
}
...
export interface TableState {
  ...
  data: StateData[];
  totalRecords: number;
  reorderHighlighter: number | null;
  page: number;
  selectAll?: {
    checked: boolean,
...
const reorderCol = (_this: Table, name: string, el: HTMLDivElement | null) => {
  ...
  function stopReorder() {
    ...
    _this.updateReorderHighlighter(null);
...
const HeaderCell = (props: HeaderCellProps) => {
  ...
  if (schema.comparator) options = [...sortOptions, ...options];
...
const TableBody = (props: TableBodyProps) => {
  ...
  if (error) return errorTemplate ? errorTemplate() : (<Heading>Couldn't fetch data</Heading>);
...
export class Table extends React.Component<TableProps, TableState> {
  ...
  constructor(props: TableProps) {
    ...
    this.state = {
      init: false,
      schema: props.schema || props.loaderSchema || [],
      data: [],
      totalRecords: 0,
      reorderHighlighter: null,
      page: 1,
      loading: true,
      sortingList: [],
      error: false
  ...
  componentDidUpdate(prevProps: TableProps, prevState: TableState) {
    if (this.props.withPagination && prevState.page !== this.state.page) {
      this.syncSelectAll();
      this.updateRenderedData();
    }
    if (prevProps.withPagination !== this.props.withPagination) {
      this.updateRenderedData();
    }
  }
  ...
  updateRenderedData = (options?: FetchDataAttr) => {
    ...
    this.getData({ page, pageSize, sortingList, ...options })
      .then(res => {
        this.setState({
          init: true,
          schema: res.schema,
  ...
  syncSelectAll = () => {
    ...
    this.updateSelectAll({
      indeterminate: data.some(d => !d._selected) && !data.every(d => !d._selected),
      checked: false
    });
  }
  ...
  fetchData: fetchDataFn = (attr: FetchDataAttr) => {
    const {
      schema
    } = this.state;
    ...
    const renderedData = withPagination && page && pageSize ?
  ...
  getData = (options: FetchDataAttr) => {
    const fetchData = this.props.fetchData || this.fetchData;
